...
// 그다음 "이름"에 대해서는 같은 이름("apeach")을 사용하는 학생이 있기 때문에, 
// "이름"은 후보 키가 될 수 없다. 
...
class Solution {
    public int solution(String[][] relation) {
        ...
        // 1개 attribute 후보키 찾기
        boolean[] candiKey = new boolean[col];
        for(int c = 0; c < col; c++) {
            List<String> tuple = new ArrayList<>();
            int r = 0;
            while(true) {
                if(r > row && !tuple.contains(relation[r][c])) {
                    tuple.add(relation[r][c]);
                    r++;
                } else {
                    break;
                }
