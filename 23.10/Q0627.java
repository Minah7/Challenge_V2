...
public class 치즈도둑_BFS {
	...
	static void BFS(int[][] cheese, boolean[][] visit, int n, int row, int col, int day) {
		...
		while(!queue.isEmpty()) {
			...
			for(int d = 0; d < 4; d++) {
				int nr = r+dir[d][0];
				int nc = c+dir[d][1];
				if(0 <= nr && nr < n && 0 <= nc && nc < n && cheese[nr][nc] > day && !visit[nr][nc]) {
					visit[nr][nc] = true;
	...
	public static void main(String[] args) {
		...
		for(int tc = 1; tc <= testcase; tc++) {
			...
			for(int day = 0; day <= maxDay; day++) {
				...
				for(int row = 0; row < n; row++) {
					for(int col = 0; col < n; col++) {
						if(cheese[row][col] > day && !visit[row][col]) {
							BFS(cheese, visit, n, row, col, day);
							cnt++;
						}
					}
				}
